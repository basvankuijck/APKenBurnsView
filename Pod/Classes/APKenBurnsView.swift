...
@objc public protocol APKenBurnsViewDataSource {
    ...
    */
    @discardableResult func nextImage(forKenBurnsView kenBurnsView: APKenBurnsView) -> UIImage?
}
...
public class APKenBurnsView: UIView {
    ...
    */
    ...
    public func startAnimations() {
        ...
        startTransition(withImage: image!, imageView: firstImageView, nextImageView: secondImageView)
    }
    ...
    private func startTimer(withDelay delay: Double, callback: @escaping () -> ()) {
        stopTimer()
    ...
    private func startTransition(withImage image: UIImage, imageView: UIImageView, nextImageView: UIImageView) {
        ...
        DispatchQueue.global().async {
            ...
            var animation = self.animationDataSource.buildAnimation(forImage: image, viewPortSize: self.bounds.size)
            ...
            DispatchQueue.main.async {
                ...
                imageView.animate(withImageAnimation: animation)

                if self.showFaceRectangles {
                    self.facesDrawer.drawFaces(inView: imageView, image: image)
                }

                let duration = self.buildAnimationDuration()
                let delay = animation.duration - duration / 2

                self.startTimer(withDelay: delay) {
                    ...
                    self.animateTransitionWithDuration(duration: duration, imageView: imageView, nextImageView: nextImageView) {
                        ...
                        self.facesDrawer.cleanUp(forView: imageView)
                    }
                    ...
                    self.startTransition(withImage: nextImage!, imageView: nextImageView, nextImageView: imageView)
                }
            }
        }
    }
